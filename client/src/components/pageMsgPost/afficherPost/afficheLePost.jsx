import React from 'react'
import CategoriePostMsg from './CategoriePostMsg';
import ContentPostMsg from './ContentPostMsg';
import TitreSelectPost from './TitreSelectPost';

export default function AfficheLePost({listePost}) {


  return (
    <>
      <TitreSelectPost/>
      <div className="allMsgs  sup768:mb-[35px] sup768:w-[700px] sup1024:w-[850px] sup1600:w-[1000px] w-[90%] mb-[20px] mt-0  mx-auto max-w-[1800px] ">
        <CategoriePostMsg/>
        <ContentPostMsg listePost={listePost}/>
      </div>
    </>
  )
}